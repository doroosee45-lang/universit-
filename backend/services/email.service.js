...
const verifyTransporter = () => {
  ...
  transporter.verify((error, success) => {
    if (error) {
      ...
      console.log('✅ SMTP SendGrid prêt à envoyer des emails');
    }
  });
};
...
const sendEmail = async ({ to, subject, html }) => {
  if (!process.env.EMAIL_PASS) {
    console.error('❌ [EMAIL] EMAIL_PASS manquant dans les variables d\'environnement');
    return { success: false, error: 'EMAIL_PASS manquant' };
  }

  try {
    const transporter = createTransporter();
    const info = await transporter.sendMail({
      from: process.env.EMAIL_FROM || '"Collège Omedev" <oseedoro@gmail.com>',
      to,
      subject,
      html
    });
