// ─── Ajout à ajouter dans services.js ───────────────────────────────────────
...
import {
  Plus, Edit, Trash2, Eye, Search, X,
  ChevronLeft, ChevronRight, Users, User, Building,
} from 'lucide-react';
...
const T = {
  ...
  emerald: '#059669', emerLt:  '#D1FAE5', emerDk: '#065F46',
...
const Spinner = ({ size = 24 }) => (
  <div style={{
    ...
    border: `2px solid ${T.line}`, borderTopColor: T.indigo,
...
/* ─── Button ─────────────────────────────────────────────────────────────── */
function Btn({ children, onClick, variant = 'primary', size = 'md', loading, disabled, type = 'button' }) {
  const V = {
  ...
  const S = {
    ...
    icon: { padding: 6, width: 32, height: 32 },
  };
  const v = V[variant];
...
const Avatar = ({ firstName, lastName, photo, size = 'sm' }) => {
  const dim = { sm: 32, md: 40, lg: 56 }[size];
  ...
  return (
    <div style={{
      ...
      color: T.white, fontWeight: 600, fontSize: dim * 0.4, flexShrink: 0,
    }}>
      {photo
...
function Field({ label, value, onChange, placeholder, type = 'text', required }) {
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      {label && (
        <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: T.slate, marginBottom: 6, letterSpacing: .4, textTransform: 'uppercase' }}>
      ...
      <input
        ...
        style={{
          ...
          background: focus ? T.indiLt : T.white, transition: 'all .18s',
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      <select value={value ?? ''} onChange={onChange} style={{
        ...
        background: T.white, outline: 'none', cursor: 'pointer', color: T.navy,
      }}>
...
function Modal({ isOpen, onClose, title, children, size = 'md' }) {
  ...
  const widths = { sm: 400, md: 580, lg: 780 };
...
function StatCard({ title, value, icon, accent }) {
  return (
    <div style={{ background: T.white, borderRadius: 14, border: `1px solid ${T.line}`, padding: '20px 22px', display: 'flex', alignItems: 'center', gap: 16, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ width: 46, height: 46, borderRadius: 12, background: accent + '18', color: accent, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        {icon}
      </div>
      <div>
...
function StaffFormModal({ staff, onSave, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        ...
        toast('Personnel mis à jour ✓');
      } else {
        await staffAPI.create({ ...form, password: form.password || 'Staff@123' });
        toast('Personnel créé ✓');
      }
  ...
  return (
    <form onSubmit={handleSubmit} style={{ padding: 24 }}>
      ...
      <Field label="Téléphone" value={form.phone} onChange={set('phone')} placeholder="Optionnel" />
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 20px' }}>
      ...
      {!isEdit && (
        <Field label="Mot de passe" type="password" value={form.password} onChange={set('password')} placeholder="Vide → 'Staff@123'" />
      )}
...
function StaffViewModal({ staff }) {
  ...
  const posLabel = POSITION_OPTS.find(p => p.value === staff.position)?.label || staff.position || '—';
  const rows = [
    ['Email',        staff.email       || '—'],
    ['Téléphone',    staff.phone       || '—'],
    ['Département',  staff.department  || '—'],
    ['Matricule',    staff.employeeId  || '—'],
    ['Statut',       staff.isActive !== false ? 'Actif' : 'Inactif'],
  ];
  return (
    <div style={{ padding: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: 16, background: T.bg, borderRadius: 12, marginBottom: 20 }}>
        <Avatar firstName={staff.firstName} lastName={staff.lastName} photo={staff.profilePhoto} size="lg" />
        <div>
          <h3 style={{ fontWeight: 700, color: T.navy, fontSize: 16, margin: 0 }}>{staff.firstName} {staff.lastName}</h3>
          <p style={{ fontSize: 13, color: T.indigo, margin: '4px 0 0' }}>{posLabel}</p>
        </div>
      </div>
      {rows.map(([label, value]) => (
        <div key={label} style={{ background: T.bg, borderRadius: 10, padding: '12px 16px', marginBottom: 10 }}>
          <p style={{ fontSize: 10, color: T.silver, margin: '0 0 4px', textTransform: 'uppercase', letterSpacing: .5, fontWeight: 600 }}>{label}</p>
          <p style={{ fontSize: 13, fontWeight: 500, color: T.navy, margin: 0 }}>{value}</p>
        </div>
      ))}
    </div>
  );
...
export default function StaffPage() {
  ...
  const fetchStaff = useCallback(async () => {
    ...
    try {
      ...
      // ✅ Le controller renvoie { success, data: [...], total }
      // staffAPI.getAll() retourne directement res.data via ApiClient
      // Donc res = { success, data: [...], total, page, pages }
      const staff = res?.data ?? [];
      const count = res?.total ?? staff.length;

      setData(staff);
      setTotal(count);
    } catch (err) {
      console.error('[StaffPage] fetchStaff:', err);
  ...
  const handleDelete = async () => {
    if (!deleteDialog.staff?._id) return;
    setDeleting(true);
    try {
  ...
  const columns = [
    {
      header: 'Personnel', key: 'firstName',
      render: (_, row) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <Avatar firstName={row.firstName} lastName={row.lastName} photo={row.profilePhoto} />
          <div>
            <p style={{ fontWeight: 600, color: T.navy, margin: 0, fontSize: 13 }}>{row.firstName} {row.lastName}</p>
            <p style={{ fontSize: 11, color: T.silver, margin: 0, fontFamily: T.mono }}>{row.employeeId || '—'}</p>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: v => {
        ...
        return <Badge bg={T.indiLt} color={T.indigo}>{pos?.label || v || '—'}</Badge>;
      },
    },
    {
  ...
  const activeCount = data.filter(s => s.isActive !== false).length;
  const deptCount   = [...new Set(data.map(s => s.department).filter(Boolean))].length;
  ...
  return (
    <div style={{ fontFamily: T.font, padding: 24, maxWidth: 1440, margin: '0 auto', minHeight: '100vh', background: '#F1F5F9' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
        <StatCard title="Total personnel" value={loading ? '…' : total}          icon={<Users size={20} />}    accent={T.indigo} />
        <StatCard title="Actifs"          value={loading ? '…' : activeCount}    icon={<User size={20} />}     accent={T.emerald} />
        <StatCard title="Départements"    value={loading ? '…' : deptCount}      icon={<Building size={20} />} accent={T.gold} />
      </div>
      ...
      <Card style={{ padding: '14px 18px', marginBottom: 20 }}>
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <Btn onClick={() => setModal({ open: true, staff: null })}>
      ...
      {/* Tableau */}
      <Card>
        <div style={{ padding: '16px 20px', borderBottom: `1px solid ${T.line}`, background: T.bg, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h2 style={{ fontSize: 13, fontWeight: 700, color: T.navy, margin: 0 }}>Liste du personnel</h2>
            <p style={{ fontSize: 11, color: T.silver, margin: '3px 0 0' }}>
              {loading ? 'Chargement…' : `${total} membre${total > 1 ? 's' : ''} enregistré${total > 1 ? 's' : ''}`}
            </p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: 56, color: T.silver }}>
            ...
            <p style={{ fontFamily: T.font, fontSize: 13 }}>
              {searchTerm ? 'Aucun résultat pour cette recherche' : 'Aucun personnel enregistré'}
            </p>
          </div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <div class="overflow-x-auto"><table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: T.font }}>
              <thead>
        ...
        {Math.ceil(total / limit) > 1 && (
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 10, padding: 16, borderTop: `1px solid ${T.line}` }}>
            ...
            <span style={{ fontSize: 12, color: T.slate, fontFamily: T.font }}>
              Page <strong>{page}</strong> sur {Math.ceil(total / limit)}
            </span>
            <button onClick={() => setPage(p => p + 1)} disabled={page >= Math.ceil(total / limit)}
              style={{ width: 32, height: 32, border: `1px solid ${T.line}`, borderRadius: 8, background: T.white, cursor: page >= Math.ceil(total / limit) ? 'not-allowed' : 'pointer', opacity: page >= Math.ceil(total / limit) ? .4 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <Modal isOpen={modal.open} onClose={() => setModal({ open: false, staff: null })}
        title={modal.staff ? 'Modifier le membre' : 'Nouveau membre du personnel'} size="md">
        <StaffFormModal
          staff={modal.staff}
          onSave={() => { setModal({ open: false, staff: null }); fetchStaff(); }}
          onCancel={() => setModal({ open: false, staff: null })}
        />
      </Modal>

      {/* Modal Visualisation */}
      <Modal isOpen={viewModal.open} onClose={() => setViewModal({ open: false, staff: null })}
        title="Profil du membre" size="sm">
        <StaffViewModal staff={viewModal.staff} />
      </Modal>
