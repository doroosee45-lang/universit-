// // pages/student/StudentSettingsPage.jsx
// import { useState, useEffect } from 'react';
// import { Save, Lock, Bell, User, Shield, AlertTriangle, X, Eye, EyeOff } from 'lucide-react';
// import { studentAPI, authAPI } from '../../services/services';
...
//   // Options pour les formulaires académiques
...
//     try {
//       const res = await studentAPI.getPrograms(); // à adapter selon votre API
...
//     try {
//       await studentAPI.updateProfile({ phone: profileForm.phone, address: profileForm.address });
//       await refreshUser();
...
//   // Sauvegarde des informations académiques (classe/promotion)
//   const saveAcademic = async () => {
//     if (!academicForm.programId || !academicForm.level || !academicForm.currentSemester) {
//       toast('Veuillez remplir tous les champs', 'error');
//       return;
//     }
//     setLoading(prev => ({ ...prev, academic: true }));
//     try {
//       await studentAPI.updateAcademic(academicForm);
//       await refreshUser();
//       toast('Informations académiques mises à jour – vos emplois du temps et notes ont été recalculés');
//     } catch (err) {
//       toast(err.message || 'Erreur', 'error');
//     } finally {
...
//     try {
//       await studentAPI.updateSettings({ notifications });
//       await refreshUser();
...
//       {/* Section Académique (classe/promotion) */}
//       <Card className="p-6">
//         <div className="flex items-center gap-2 mb-4">
//           <Shield size={20} className="text-indigo-600" />
//           <h2 className="text-lg font-semibold text-gray-800">Classe & Promotion</h2>
//         </div>
//         <p className="text-sm text-gray-500 mb-4">
//           La modification de votre filière ou niveau mettra automatiquement à jour votre emploi du temps, vos notes et vos examens.
//         </p>
//         <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
//           <Select
//             label="Filière"
//             value={academicForm.programId}
//             onChange={e => handleAcademicChange('programId', e.target.value)}
...
//             onChange={e => handleAcademicChange('level', e.target.value)}
...
//           />
//         </div>
//         <Button onClick={saveAcademic} loading={loading.academic} className="mt-2"><Save size={16} /> Mettre à jour ma classe</Button>
//       </Card>
...
// pages/student/StudentSettingsPage.jsx
import { useState, useEffect } from 'react';
import { Save, Lock, Bell, User, Shield, AlertTriangle, X } from 'lucide-react';
import { studentAPI, authAPI } from '../../services/services';
import { useAuth } from '../../components/context/AuthContext';
...
const Card = ({ children, className = '' }) => (
  <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E5E7EB', padding: 24, marginBottom: 20 }} className={className}>
    {children}
  </div>
);
...
const Input = ({ label, value, onChange, placeholder, type = 'text', required, error }) => (
  <div style={{ marginBottom: 16 }}>
    ...
    <input type={type} value={value ?? ''} onChange={onChange} placeholder={placeholder}
      required={required} style={inputStyle(error)}
...
export default function StudentSettingsPage() {
  ...
  const [loading, setLoading] = useState({
    profile: false, academic: false, password: false,
  ...
  // Sync form si user change (ex: après refreshUser)
  useEffect(() => {
    if (!user) return;
    setProfileForm({ phone: user.phone || '', address: user.address || { street: '', city: '', wilaya: '' } });
    setAcademicForm({ programId: user.program?._id || '', level: user.level || '', currentSemester: user.currentSemester || '' });
    setNotifications({
      email: user.settings?.emailNotifications !== false,
      push:  user.settings?.pushNotifications  !== false,
      sms:   user.settings?.smsNotifications   || false,
  ...
  const semesterOptions = [
    ...
    { value: 'S3', label: 'Semestre 3' }, { value: 'S4', label: 'Semestre 4' },
    { value: 'S5', label: 'Semestre 5' }, { value: 'S6', label: 'Semestre 6' },
  ];
  ...
  const saveAcademic = async () => {
    ...
    try {
      await studentAPI.updateAcademic(academicForm);
      await refreshUser();
      toast('Informations académiques mises à jour');
    } catch (err) {
      toast(err?.message || 'Erreur', 'error');
    } finally { setLoad('academic', false); }
  };
  ...
  return (
    <div style={{ maxWidth: 768, margin: '0 auto', padding: '0 16px' }}>
      <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
      <ToastContainer />
      <h1 style={{ fontSize: 22, fontWeight: 700, color: '#111827', marginBottom: 24 }}>Paramètres du compte</h1>
      ...
      {/* Classe & Promotion */}
      <Card>
        <SectionHeader icon={Shield} title="Classe & Promotion" />
        <p style={{ fontSize: 13, color: '#6B7280', marginBottom: 16 }}>
          La modification de votre filière ou niveau mettra automatiquement à jour votre emploi du temps, vos notes et vos examens.
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12 }}>
          <Select label="Filière"  value={academicForm.programId}      onChange={e => setAcademicForm(p => ({ ...p, programId: e.target.value }))}       options={programOptions} required />
          <Select label="Niveau"   value={academicForm.level}          onChange={e => setAcademicForm(p => ({ ...p, level: e.target.value }))}            options={levelOptions}   required />
          <Select label="Semestre" value={academicForm.currentSemester} onChange={e => setAcademicForm(p => ({ ...p, currentSemester: e.target.value }))} options={semesterOptions} required />
        </div>
        <Button onClick={saveAcademic} loading={loading.academic}><Save size={15} /> Mettre à jour ma classe</Button>
      </Card>
